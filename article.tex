...
\SubSection{Tags and Tagging}

%   Applying tags (to dataset)
%   Tagging
%     Group of tags
...
%       type etc.
  ...
  all articles on birds
  all articles on africa
  all articles on african birds
  all articles on birds from africa
...
% go through different folder setup
% then insert the text about ``Tags, on the other hand..''
% elaborate what it means that an object can have many tags
% show a venn diagram
% explain the venn diagram
  ...
  Tags, on the other hand, are \emph{non-hierarchical} and \emph{inclusive},
  which is to say that an object can be associated with a great variety of
  tags simultaneously\cite{golder:ct}.
  ...
  % Finally we can define tags as being non-hierarchical..
  \emph{Tags} are non-hierarchical descriptive terms, keywords or labels that
  is attached to an object for later
  retrieval\cite{golder:ct}\cite{huang:tt}\cite{shirky:ontology}\cite{wikipedia:tags}.
  ...
  %(mention polyhierarchical from the IA book)


  %Descriptive terms, keywords or labels are metadata associated with a given
  %object used to describe a concept or type of information\cite{heymann:ccch}.

  %Object.. (define an object, then say that in this article an object is a
  %pattern movement || a movement in a pattern)
...
%   non-hierarchical
%   descriptive terms, keywords or labels
%   object (a movement in a pattern)
...
%\emph{Tagging} can be defined as the practice of creating and managing labels
%that categorize content using simple keywords


Tagging is the act of organizing a collection of entities, whether things or
concepts, into related groups\cite{shirky:ontology}.
