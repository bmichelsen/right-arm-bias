...
\begin{em}
  ...
  the encyclopedia is that there should be an equal amount of techniques to
  both sides of the body. Here we analyze hand techniques in Chon-Ji through
  Juche to establish whether or not there is a bias towards either side.  This
  is done by attaching metadata, in form of tags, to each pattern movement.
  These tags are extracted from each movement's English description in the
  condensed encyclopedia, and supplemented with keywords noted while
  performing the pattern. We then apply concepts from set theory to retrieve
  information we are interested in, and find that there are seven more
  techniques on the right side than on the left.
...
\SubSection{Techniques by Patterns}
  ...
  the same trend, where there are most blocks, then there are more punches
  than strikes, and more strikes than thrusts (Figure \ref{fig:patterns_all}).
  ...
  blocks than punches), Choong-Moo, Choong-Jang and Juche (with more punches
  than strikes) and Po-Eun (where there are more strikes than thrusts).
...
\Section{Conclusions}
  ...
  side. Thusly, there is a bias towards the right side.
