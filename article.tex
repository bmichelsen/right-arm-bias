...
\SubSection{Approach}
  ...
  side (left, right or none where applicable --- ie. sitting stance),
  technique height (low, middle, high), technique side (left arm or right arm,
  can also be both), direction of the technique (when needed), primary
  technique type (attack, block, kick), technique group (punch, strike,
  thrust), tool group (forearm, knife hand, finger etc.), tool (inner forearm,
  outer forearm, double finger etc.), and finally motion type (normal motion,
  slow motion, consecutive motion and so on).
