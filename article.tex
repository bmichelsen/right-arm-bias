...
\SubSection{Approach}
  ...
  from the condensed encyclopedia. In addition, suitable tags is attached to
  the dataset while performing the pattern to add necessary detail.
  ...
  \begin{align*}
    ...
    left \: technique, \: block, \: forearm, \: outer \: forearm, \\
    normal \: motion\}
  \end{align*}

  Information about the stance type is presented first, followed by stance
  side (left, right or none where applicable --- ie. sitting stance, technique
  height (low, middle, high), technique side (left arm or right arm, can also
  be both), direction of the technique (when needed), primary technique type
  (attack, block, kick), technique group (punch, strike, thrust), tool group
  (forearm, knife hand, finger etc.), tool (inner forearm, outer forearm,
  double finger etc.), and finally motion type (normal motion, slow motion,
  consecutive motion and so on).

  % FIXME add database and ruby script info here
  ...
  database, enables us to retrieve the data we're interested in using concepts
  from set theory.


\SubSection{Classification}

% TODO: vurderinger på teknikker, se notater i wikien
...
\begin{itemize}
  ...
  \item
    ...
    \begin{itemize}
      \item
        Free software is available for experimentation and testing.
      \item
        Most readers of this text have a background in relational technology. 
    \end{itemize}
\end{itemize}
