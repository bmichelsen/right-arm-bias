...
\SubSection{Set theory}
  ...
  The characteristics of tags make them excellent for future navigation,
  filtering or search. An object which has been tagged contains a collection,
  or a set, of tags.  Each tag is an element of the set. The order of elements
  within the set is irrelevant, but each element can only occur once.

  For example, $\{1,5,8\}$ denotes a set with three elements: 1, 5 and 8.
  ...
  several times. Elements in a set can also contain strings, so $\{loons,
  penguins, terns\}$ is a valid set.
...
\SubSection{Approach}
% collecting data into text files
