...
\SubSection{Techniques by Groups}

% what are the groups of techniques?


  Table 2: Percent distribution by arm per category
  ...
  In terms of category distribution, there are more blocks with the left side
  than there are with the

  It is worth noting that there are more blocks with the left side than with
  the right side. That is, there are more attacks with the right arm than with
  the left arm.
...
\SubSection{Techniques by Height}
  ...
  Techniques by height follow the same pattern when looking at all techniques,
  as well as both sides. Most of the techniques are in the middle section,
  except for strikes which there are more of in the high section. There are
  very few punches, strikes and thrusts to the low section. In general, there
  are more blocks than any other technique.
...
\SubSection{Techniques by Stances}
  ...
  % All techniques by stances except walking, sitting and L
  \begin{figure}
...
\SubSection{Techniques by Motion Type}
  ...
  Most techniques are normal motion, followed by fast motion and continuous
  motion.

  There are more motion types on the right side, than on the left. Right side
  has flying motion and consecutive motion, whereas the left side does not.
  There are more flying, slow, sliding and consecutive motions on the right
  side. On the left side, there are more fast, stamping continuous and
  connecting motions than on the right side.

  There are equal amounts of slow, jumping, dodging, double stepping, shifting
  motions on both sides, although with different groups of techniques.
...
% right hand, than with the right?
