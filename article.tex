...
\Section{Introduction}
  ...
  to Juche using tags and then report our findings of a left arm bias.
...
\SubSection{Set theory}
  A \emph{set} is a collection of distinct objects. It is a plurality thought
  of as a unit\cite{hausdorff:sets}. \emph{Set theory} is the branch of
  mathematics that studies sets\cite{wikipedia:tags}.
  ...
  repeatedly. Elements in a set can contain strings as well, so $\{loons,
  terns, penguins\}$ is also a valid set.
...
\Section{Method}
  ...
  In this section we describe where the data originates, and then elaborate on
  the approach taken to register and investigate the dataset.
...
\SubSection{The Data}
  ...
  The terminology dataset is organized by pattern names. It contains a list of
  all techniques in patterns Chon-Ji to Juche using terminology. It also
  includes motion type on movements that are not performed in normal motion.

  The English description to every movement was registered manually after
  reviewing, and making minor corrections to, the terminology.


\SubSection{Approach}

  Each pattern movement is tagged on the basis of their English description
  from the condensed encyclopedia. In addition, suitable tags were attached to
  the dataset while performing the pattern to add necessary details.
  ...
  with the left forearm.'' Removing the unnecessary parts of the description,
  we're left with:
...
% why?
%because it to tags means we need to be more specific, so it's easier to
%retrieve movements of same qualities..
 

% pek på forskjeller mellom tags og bevegelse, hvilke detaljer som blir
% utelatt, detaljer i forhold til diagrammet er ikke tatt med i datasettet




% techniques were grouped

%stance[type, (side)], technique[height,


%stance type, [stance side], technique height, technique side, [direction of
%technique side], technique type, tool group, technique tool,
%[specific technique tool], motion type

%stance (type, [side]), technique (height, side, [direction], type), technique
%(tool group, tool, [specific tool], motion type


%stance type
%stance side (left, right, none (sitting stances for example))
%technique height
%technique side (left, right or both)
%[direction of technique (side_front)]
%technique type (attack or block)
%tool group (forearm, strike, thrust, punch)
%technique tool (outer_forearm, rising_block, front_strike, finger, angle_punch)
%specific technique tool, when needed (side_elbow, double_finger)
%motion type (normal_motion, fast_motion)


%ett eksempel med blokk, pluss ett eksempel med angrep (begge eksemplene bør få
%med de aller fleste momentene nevnt over for å illustrere hva som menes med de
%ulike gruppene)
...
% Tagging
%   tagging (how is tagging used? what is its role?)
%   connect set theory to tagging? how is it applied to tags and tagging?
